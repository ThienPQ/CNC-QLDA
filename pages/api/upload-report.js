// pages/api/upload-report.js (Phiên bản cuối cùng, chống lỗi khi đọc PLHD)
...
async function handleContractUpload(filePath) {
  console.log("[PLHD] Bắt đầu xử lý file kế hoạch...");
  ...
  console.log(`[PLHD] Đã đọc được ${contractData.length} dòng từ sheet '${sheetName}'.`);
  ...
  console.log("[PLHD] Đã dọn dẹp các bảng cũ.");
...
async function handleWeeklyReportUpload(filePath, fields) {
  console.log("[BCT] Bắt đầu xử lý báo cáo tuần...");
  // ... (Nội dung hàm này giữ nguyên như cũ, không cần thay đổi)
